...
def cli_pkcs12_tests(tmp_dir):
    ...
    if not os.path.exists(out_key_enc):
        logging.error("pkcs12_import --key-pass: key file not written")
    else:
        key_pem = open(out_key_enc, encoding='utf-8').read()
        if "ENCRYPTED" not in key_pem:
            logging.error("pkcs12_import --key-pass: output key is not encrypted")
