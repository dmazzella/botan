 ...
 */
struct BOTAN_PUBLIC_API(3, 12) PKCS12_Options {
      /**
      ...
      /**
       * Number of iterations for the PKCS#12 KDF (default: 100000).
       * Values below 10000 are not recommended.
       * For maximum compatibility with legacy software, use 2048.
       */
      size_t iterations = 100000;

      /**
       * Encryption algorithm for the private key.
       * Supported: "PBES2-SHA256-AES256" (default, modern, requires PBES2 module),
       *            "PBES2-SHA256-AES128" (modern, requires PBES2 module),
       *            "PBE-SHA1-3DES" (legacy, widest compatibility),
       *            "PBE-SHA1-2DES" (legacy)
       */
      std::string key_encryption_algo = "PBES2-SHA256-AES256";

      /**
       * Encryption algorithm for certificates.
       * If empty, certificates are stored unencrypted (shrouded in Data).
       * Accepts the same values as key_encryption_algo.
       */
      ...
      /**
       * Digest algorithm for the PKCS#12 MAC (default: "SHA-256").
       * Use "SHA-1" for maximum compatibility with legacy software.
       * Supported: "SHA-1", "SHA-256", "SHA-384", "SHA-512"
       */
      std::string mac_digest = "SHA-256";

      /**
       * If true, override key_encryption_algo, mac_digest, and iterations with
       * legacy-compatible values ("PBE-SHA1-3DES", "SHA-1", 2048) regardless of
       * what was set above. Use when interoperability with old software is required.
       */
 ...
 */
class BOTAN_PUBLIC_API(3, 12) PKCS12_Data {
   public:
      PKCS12_Data() = default;
      ...
      /**
       * @return the localKeyId bag attribute of the private key, or empty if absent
       */
      const std::vector<uint8_t>& local_key_id() const { return m_local_key_id; }

      /**
       * @return true if a localKeyId attribute is present on the private key
       */
      bool has_local_key_id() const { return !m_local_key_id.empty(); }

      /**
       * @return OIDs of bag types encountered during parsing but not handled
       *         (e.g. SecretBag). Empty in normal files.
       */
 ...
 * @endcode
 */
class BOTAN_PUBLIC_API(3, 12) PKCS12 final {
   public:
      /**
