...
std::pair<secure_vector<uint8_t>, secure_vector<uint8_t>> pkcs12_derive_key_iv(std::string_view password,
   ...
   const secure_vector<uint8_t> pwd_bytes = pkcs12_encode_password(password);
   secure_vector<uint8_t> key(params.key_len);
   secure_vector<uint8_t> iv(iv_len);
   pkcs12_kdf(key.data(), params.key_len, pwd_bytes, salt.data(), salt.size(), iterations, 1);
   pkcs12_kdf(iv.data(), iv_len, pwd_bytes, salt.data(), salt.size(), iterations, 2);
