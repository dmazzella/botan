...
class PKCS12_Tests final : public Test {
   ...
   private:
      ...
      struct TestCredentials {
            std::unique_ptr<Botan::ECDSA_PrivateKey> key;
      ...
      TestCredentials generate_credentials(Botan::RandomNumberGenerator& rng,
         ...
         creds.key = std::make_unique<Botan::ECDSA_PrivateKey>(rng, Botan::EC_Group::from_name("secp256r1"));
      ...
      void verify_parsed_data(Test::Result& result,
         ...
         if(parsed.private_key()) {
            result.test_str_eq("Key algorithm", parsed.private_key()->algo_name(), "ECDSA");
      ...
      Test::Result test_roundtrip_with_chain() {
         ...
         // Generate CA
         const Botan::ECDSA_PrivateKey ca_key(*rng, Botan::EC_Group::from_name("secp256r1"));
         ...
         // Generate end-entity
         const Botan::ECDSA_PrivateKey ee_key(*rng, Botan::EC_Group::from_name("secp256r1"));
      ...
      Test::Result test_chain_with_pbes2() {
         ...
         const Botan::ECDSA_PrivateKey int_key(*rng, Botan::EC_Group::from_name("secp256r1"));
      ...
      Test::Result test_wrong_password() {
         ...
         const auto pfx = Botan::PKCS12::create(*creds.key, creds.cert, opts, *rng);

         result.test_throws<Botan::Invalid_Authentication_Tag>("wrong password throws",
                                                               [&]() { Botan::PKCS12::parse(pfx, "wrong"); });
      ...
      Test::Result test_corrupted_pfx() {
         ...
         auto pfx = Botan::PKCS12::create(*creds.key, creds.cert, opts, *rng);

         // Corrupt bytes in the middle of the data
         pfx[pfx.size() / 2] ^= 0xFF;

         result.test_throws("corrupted pfx throws", [&]() { Botan::PKCS12::parse(pfx, "test"); });
      ...
      Test::Result test_no_mac() {
         ...
         Botan::PKCS12_Options opts;
         opts.password = "nomactest";
         opts.include_mac = false;
         opts.key_encryption_algo = "PBE-SHA1-3DES";
         opts.iterations = 2048;
         ...
         const auto parsed = Botan::PKCS12::parse(pfx, "nomactest");
      ...
      Test::Result test_empty_password() {
         ...
         const auto parsed = Botan::PKCS12::parse(pfx, "");
         result.test_is_true("Has private key", parsed.has_private_key());
         result.test_is_true("Has certificate", parsed.has_certificate());
      ...
      Test::Result test_cert_only() {
         ...
         Botan::PKCS12_Options opts;
         opts.password = "certonly";
         opts.key_encryption_algo = "PBE-SHA1-3DES";
         opts.mac_digest = "SHA-1";
         opts.iterations = 2048;

         const auto pfx = Botan::PKCS12::create(nullptr, &creds.cert, {}, opts, *rng);
         result.test_sz_gt("PFX data generated", pfx.size(), 0);

         const auto parsed = Botan::PKCS12::parse(pfx, "certonly");
         result.test_is_false("No private key", parsed.has_private_key());
         result.test_is_true("Has certificate", parsed.has_certificate());
      ...
      Test::Result test_empty_input() {
         ...
         result.test_throws<Botan::Decoding_Error>("empty bytes throws",
                                                   []() { Botan::PKCS12::parse(std::span<const uint8_t>{}, "pass"); });
      ...
      Test::Result test_zero_iterations() {
         ...
         result.test_throws<Botan::Invalid_Argument>(
            "zero iterations throws", [&]() { Botan::PKCS12::create(*creds.key, creds.cert, opts, *rng); });
      ...
      Test::Result test_legacy_compat_flag() {
         ...
         Botan::PKCS12_Options opts;
         opts.password = "legacytest";
         opts.legacy_compat = true;

         const auto pfx = Botan::PKCS12::create(*creds.key, creds.cert, opts, *rng);
         result.test_sz_gt("PFX data generated", pfx.size(), 0);

         const auto parsed = Botan::PKCS12::parse(pfx, "legacytest");
         result.test_is_true("Has private key", parsed.has_private_key());
         result.test_is_true("Has certificate", parsed.has_certificate());
      ...
      Test::Result test_mac_sha256() {
         ...
         Botan::PKCS12_Options opts;
         opts.password = "mactest";
         opts.mac_digest = "SHA-256";
         opts.key_encryption_algo = "PBES2-SHA256-AES256";
         opts.iterations = 2048;
         ...
         const auto parsed = Botan::PKCS12::parse(pfx, "mactest");
         result.test_is_true("Has private key", parsed.has_private_key());
         result.test_is_true("Has certificate", parsed.has_certificate());
