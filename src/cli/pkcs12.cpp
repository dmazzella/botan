...
class PKCS12_Export final : public Command {
   public:
      PKCS12_Export() :
            Command(
               "pkcs12_export --pass= --key-pass= --friendly-name= --key-cipher=PBES2-SHA256-AES256 --no-mac --cert-cipher= --mac-digest=SHA-256 --iterations=100000 key cert *ca_certs") {
      }
      ...
      void go() override {
         ...
         options.key_encryption_algo = get_arg("key-cipher");
